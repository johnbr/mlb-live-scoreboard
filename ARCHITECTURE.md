...
| `site.web.api.espn.com/apis/common/v3/sports/baseball/mlb/athletes/<id>/stats?category=batting` | batter season stats (at-bat HR / RBI / AVG) — `category=batting` so two-way players listed as pitchers (e.g. Ohtani) still return their `career-batting` line | 60 s (`BATTER_SEASON_STATS_TTL_SECONDS`)                    |
