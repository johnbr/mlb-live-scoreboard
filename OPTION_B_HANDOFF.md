...
> **Status:** Chunks 0–5 DONE. Manual verify (2026-05-17, real HA): Chunk 2
> WebSocket round-trip ✅ and popup open + career/bio render ✅. Still
> pending before the PR: popup **keyboard a11y** (focus-trap, Esc + focus
> restore), **backdrop-click / scroll-lock**, **light/dark theme**,
> **two-card duplicate-id**, and the **`espn` direct-open path** — all fold
> into Chunk 6's a11y re-pass (recipes in §6). Only Chunk 6 (polish + open
> the PR) remains. Branch `feat/player-career-card`, based on `main` @ v1.10.0.
